...
def BigMoneyIndex_trading_signal():
    # Enter your Facebook email and password
    email = 'woodyalgotrade@gmail.com'
    password = 'algotradewoody'
    # connect to Facebook
    fps = Fps(email, password, post_url_text='Full Story')
    # Get Reaction from latest voting (Only search for latest '大戶指數')
    reaction = fps.get_latest_post_reaction()

    bullish_signal = reaction['Haha']
    bearish_signal = reaction['Sad']
    total_signal = bullish_signal + bearish_signal
    bullish_signal_ratio = bullish_signal/total_signal
    signal = None
    if bullish_signal > 0.6:
        signal = 'Short'
    elif bullish_signal > 0.5:
        signal = 'Long'
    elif bullish_signal < 0.4:
        signal = 'Long'
    elif bullish_signal <0.5:
        signal = 'Short'
...
def place_ib_order(share):
    """Place an IB Auction order"""
    app = TestApp("127.0.0.1", 7496, 1)

    ## lets get prices for this
    ibcontract = IBcontract()
    ibcontract.secType = "STK"
    ibcontract.symbol = "2800"
    ...
    order1.totalQuantity=abs(share)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        # Get Currnet Time
        if not Today_signal_check:
            # Before Market Open, Check The time every 2 minutes
            time.sleep(120)
        if hour > 9 and minute >10 and not Today_signal_check:
            # After 9:10, check Calvin Facebook and calculate the signal
            signal = BigMoneyIndex_trading_signal()
            Today_signal_check = True
            # Place an Auction Order during Market Auction Open based on signal
            if signal == 'Long':
                place_ib_order(order_shares)
                ordered = True
            elif signal == 'Short':
                place_ib_order(order_shares*-1)
                ordered = True
            # The Programme will sleep for 400 min and wake before market closed around 3:50pm
            time.sleep(400*60)

        if hour > 16:
            # When the regular trading is end, place an close order during closing auction
            if ordered:
                if signal == 'Long':
                    place_ib_order(order_shares*-1)
                elif signal == 'Short':
            ...
            ordered = False
            Today_signal_check = False
            # after placing order, the computer will sleep until next morning
            time.sleep(1000*60)
        elif hour > 15:
            # Check the time every minute after 3pm
            time.sleep(60)